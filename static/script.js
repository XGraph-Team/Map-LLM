...
// Secondary cables
    ...
    function updateTime() {
        ...
        // Fetch current simulation time from scenario controller
        fetch('/api/scenario/status')
            .then(response => response.json())
            .then(data => {
                if (data.success && data.time_formatted) {
                    // Display HH:MM:SS format from scenario controller
                    timeEl.textContent = data.time_formatted;

                    // Also update time description if element exists
                    const timeDescEl = document.getElementById('time-description');
                    if (timeDescEl && data.time_description) {
                        timeDescEl.textContent = data.time_description;
                    }
                } else {
                    // Fallback to real-world time if scenario controller not available
                    const now = new Date();
            ...
            .catch(() => {
                // If API fails, show real-world time
                const now = new Date();
                timeEl.textContent = now.toLocaleTimeString('en-US', {
                    hour: '2-digit',
                    minute: '2-digit',
